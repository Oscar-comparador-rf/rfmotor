...
@router.post("/")
async def comparar_ativos(payload: ComparacaoInput, usuario = Depends(verificar_token)):
    ...
    for a in payload.ativos:
        ...
        resultados.append({
            "emissor": a.emissor, "tipo": a.tipo,
            "indexador": a.indexador, "taxa": a.taxa,
            "prazo_dias": a.prazo_dias, "fgc": a.fgc,
            "fidc_rating": a.fidc_rating,
            "bruto": r.bruto, "ir": r.ir,
            "liquido": r.liquido, "juro_real": r.juro_real,
            "vol": r.vol, "sharpe": r.sharpe,
            "benchmark_sharpe": r.benchmark_sharpe,
            "adequacao": r.adequacao, "motivo": r.motivo,
            "equiv_cdi_pct": r.equiv_cdi_pct,
            "equiv_ipca_sp": r.equiv_ipca_sp,
            "melhor": False, "melhor_tipo": None,
        })
...
@router.post("/mtm")
async def calc_mtm(payload: MTMInput, usuario = Depends(verificar_token)):
    return marktomarket(
